...
uint256 hashGenesisBlock("0x8ecb751dab8957e69c56283d6b0e6770915b054f79b1cb5a97ddbfba0a9bc23f");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Dwarfcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Dwarfcoin Signed Message:\n";
...
{
    // Dwarfcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Dwarfcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Dwarfcoin: 1 day
static const int64 nTargetSpacing = 2 * 60; // Dwarfcoin: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Dwarfcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Dwarfcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Dwarfcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x8ecb751dab8957e69c56283d6b0e6770915b054f79b1cb5a97ddbfba0a9bc23f");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "NY Times 14/Jan/2014 Obama Fights a Push to Add Iran Sanctions";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 1000 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("040184710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a0") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1389722331;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 389629295;
        ...
        assert(block.hashMerkleRoot == uint256("0x2b6de5ab0e9ab0f76e102b1aa0a61ed8996e5be826c2dba3b99e0208b3afac9b"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Dwarfcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DwarfcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DwarfcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static DwarfcoinMiner(CWallet *pwallet)
{
    printf("DwarfcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("dwarfcoin-miner");
    ...
    try { loop {
        ...
        printf("Running DwarfcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("DwarfcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DwarfcoinMiner, pwallet));
}
