...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Dwarfcoin " + FormatFullVersion();
...
static const char *strMainNetDNSSeed[][2] = {
      {"dwarfcoin.org", "dnsseed.dwarfcoin.org"},
      {"dwarfcoin.org", "dnsseed2.dwarfcoin.org"},
      {NULL, NULL}
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Dwarfcoin is probably already running."), addrBind.ToString().c_str());
        else
